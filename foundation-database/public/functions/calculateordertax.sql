...
BEGIN
  ...
  SELECT dochead_number, dochead_taxzone_id, addr_line1, addr_line2, addr_line3, addr_city,
         ...
         dochead_tocountry, COALESCE(cust_number, prospect_number), COALESCE(taxreg_number, ' '),
         dochead_curr_id, dochead_date, dochead_freight, dochead_misc, dochead_freight_taxtype_id,
         dochead_misc_taxtype_id, dochead_misc_discount
    INTO _number, _taxzoneid, _fromline1, _fromline2, _fromline3, _fromcity,
         ...
         _currid, _docdate, _freight, _misc, _freighttaxtype,
         _misctaxtype, _miscdiscount
    FROM dochead
    ...
    LEFT OUTER JOIN taxreg ON taxreg_rel_type = 'C'
                          AND dochead_cust_id = taxreg_rel_id
